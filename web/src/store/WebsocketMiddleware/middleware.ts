...
import {
  ...
  getUserToken,
  pushSnackbarError,
} from '../AppStore/utility';
import {
  StoreType,
  WebSocketOutgoingMessage,
  WebsocketMiddleware,
  WebsocketMiddlewareImpl,
  WebsocketMiddlewareType,
} from './type';
import {
  isContactSelected,
  pushDirectMessageNotification,
  pushGroupMessageNotification,
} from './utility';
...
const reduceMessage = async (
  ...
  switch (message.type) {
    ...
    case 'GROUP_MESSAGE_NOTIFICATION': {
      const groupContact = get().groupContacts.find(
        gc => gc.id === message.groupId && gc.type === 'GROUP'
      );
      if (!groupContact) {
        logError(
          'Contact not found when websocket GROUP_MESSAGE_NOTIFICATION received'
        );
        return pushSnackbarError(set, '[Websocket] Contact not found');
      }
      if (
        !isContactSelected(get, groupContact) &&
        get().loggedInUserId !== message.senderId
      ) {
    ...
    case 'MESSAGE_NOTIFICATION': {
      ...
      const contact = get().contacts.find(
        c => c.id === contactId && c.type === 'DIRECT'
      );
      if (!contact) {
        logError(
          'Contact not found when websocket MESSAGE_NOTIFICATION received'
        );
        return pushSnackbarError(set, '[Websocket] Contact not found');
      }
      pushDirectMessageNotification(set, get, message, contact);
      if (
        !isContactSelected(get, contact) &&
        get().loggedInUserId !== message.senderUid
      ) {
    ...
    case 'READ_NOTIFICATION': {
      ...
      messageMap[`DIRECT-${contactId}`] = directMessages.map(m =>
        m.isUser ? { ...m, receiverRead: true } : m
      );
...
const connect = (
  ...
  const wsConn = new WebSocket(
    `${WebSocketEndpoint()}${Endpoint.webSocket(token)}`
  );
  ...
  store.websocket.sendMessage = (
    receiverId: number,
    message: string,
    attachments
  ) => {
    const attReq = attachments.map(at => ({
      name: `f-${at.id}`,
      contentBase64: at.file.split(',')[1],
    }));
    wsConn.send(
      JSON.stringify({
        type: 'SEND_MESSAGE',
  ...
  store.websocket.sendGroupMessage = (
    groupId: number,
    message: string,
    attachments
  ) => {
  ...
  store.websocket.editDirectMessage = (
    messageId: number,
