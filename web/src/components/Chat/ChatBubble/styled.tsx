...
const SC__ChatBubbleContainer = styled.div<SC__ChatBubbleContainerProps>`
  ...
  background-color: ${props =>
    props.$side === 'left'
      ...
      : props.$rightSideBackgroundColor};
  color: ${props =>
    props.$side == 'left' ? props.$leftSideColor : props.$rightSideColor};
  gap: 5px;
...
export const TH__Button: DivWrapper<TH__ButtonProps> = ({
  children,
  ...props
}) => {
...
type TH__ChatBubbleContainerProps = PropsWithChildren<
  DivProps & {
    $side: Side;
  }
>;

export const TH_ChatBubbleContainer: FC<TH__ChatBubbleContainerProps> = ({
  children,
  $side,
  ...props
}) => {
  const rightSideBackgroundColor =
    useColorConfig().chatBubbleBackgroundColorUserSent;
