...
const ToggleShowPasswordButton = styled.button<{ $showing: boolean }>`
    ...
    outline: none;
    border-radius: 4px;
    background-color: ${props => props.$showing ? DarkTheme.config.chatListNavSearchInnerBackgroundColor : 'transparent'};
    cursor: pointer;
    display: flex;
...
export const InputField: FC<InputFieldProps> = (props) => {
    const onKeyPressInInput = useCallback((e: KeyboardEvent<HTMLInputElement>) => {
        e.stopPropagation();
        if (e.key === "Enter") {
            props.onClickEnter?.();
            e.preventDefault();
