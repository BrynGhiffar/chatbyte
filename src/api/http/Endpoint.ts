...
export const BackendEndpoint = (): string | null => {
  ...
  const base_path = z
    .string()
    .safeParse(import.meta.env.VITE_BACKEND_BASE_PATH);
...
export class Endpoint {
  static getMessage = (receiverUid: number) =>
  ...
  static groupMessage = (groupId: number) =>
    `/message/group?groupId=${groupId}`;
  static groupAvatar = (groupId: number) => `/group/image/${groupId}`;
  static attachmentImgSrc = (attId: number) => `/attachment/${attId}`;
}

export const AttachmentSrc = (attId: number) =>
  `${BackendEndpoint()}${Endpoint.attachmentImgSrc(attId)}`;
...
export const request = async <T extends z.ZodTypeAny>(
  parser: T,
  request: RequestWithBody | Request
): Promise<
  { success: true; payload: z.infer<T> } | { success: false; message: string }
> => {
  ...
  if (!safeParse.success)
    return {
      success: false,
      message: `FAILED_PARSING_RESPONSE: ${JSON.stringify(json)}`,
    };
